import type p5 from 'p5';
import type { setupCallback, valueCallback } from '../../../types';
import type { Controller } from '../Controller';
import type { CatalystGUI } from '../../CatalystGUI';
import { ValuedController } from '../ValuedController';

/**
 * Side by side incrementer & decrementer button for a number
 * @extends ValuedController
 */
export class Crementer extends ValuedController {
	minVal: number;
	maxVal: number;
	stepSize: number;
	valueDisplay: p5.Element;
	/**
	 * The value callback.
	 * @type {valueCallback}
	 */
	valueCallback: valueCallback;

	/**
	 * Crementer constructor
	 * @param {CatalystGUI} gui
	 * @param {string} name
	 * @param {string} labelStr
	 * @param {number} minVal
	 * @param {number} maxVal
	 * @param {number} defaultVal
	 * @param {number} stepSize
	 * @param {ValueCallback} valueCallback
	 * @param {SetupCallback} [setupCallback]
	 */
	constructor(
		gui: CatalystGUI,
		name: string,
		labelStr: string,
		minVal: number,
		maxVal: number,
		defaultVal: number,
		stepSize: number,
		valueCallback?: valueCallback,
		setupCallback?: setupCallback
	) {
		super(gui, name, labelStr, defaultVal, setupCallback);
		this.minVal = minVal;
		this.maxVal = maxVal;
		this.stepSize = stepSize;
		this.valueCallback =
			valueCallback || ((controller: Controller, value: any) => {});

		this.controllerElement = gui.sketch.createDiv();
		this.controllerElement.class('crementer');
		this.controllerElement.parent(this.controllerWrapper);

		const minusButton = gui.sketch.createButton('&#x2190'); // left arrow
		minusButton.parent(this.controllerElement);
		minusButton.elt.onclick = () => this.decrement();

		this.valueDisplay = gui.sketch.createSpan(defaultVal.toString());
		this.valueDisplay.parent(this.controllerElement);

		const plusButton = gui.sketch.createButton('&#x2192'); // right arrow
		plusButton.parent(this.controllerElement);
		plusButton.elt.onclick = () => this.increment();

		this.valueCallback(this, this.value);
	}

	mod(value: number) {
		const modSize = this.maxVal - this.minVal + 1; // [min,max] inclusive
		return ((value - this.minVal + modSize) % modSize) + this.minVal;
	}

	increment() {
		this.setValue(this.mod((this.value as number) + this.stepSize));
	}

	decrement() {
		this.setValue(this.mod((this.value as number) - this.stepSize));
	}

	setValue(value: number) {
		this.value = this.gui.sketch.constrain(value, this.minVal, this.maxVal);
		this.valueDisplay.html(this.value.toString());
		this.valueCallback(this, this.value);
		if (this.doUpdateChangeSet()) this.gui.changeSet.save();
	}

	randomize() {
		let randomValue = this.gui.sketch.random(this.minVal, this.maxVal);
		randomValue =
			this.gui.sketch.round(randomValue / this.stepSize) * this.stepSize;
		this.setValue(randomValue);
	}
}
